...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseMain() {
		...
		// <常量声明>
		auto err_con = analyseConstantDeclaration();
		if (err_con.has_value())
			return err_con;
		// <变量声明>
		auto err_val = analyseVariableDeclaration();
		if (err_val.has_value())
			return err_val;
		// <语句序列>
		auto err_stm = analyseStatementSequence();
		if (err_stm.has_value())
			return err_stm;
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
		...
		while (true) {
			...
			// 变量可能没有初始化，仍然需要一次预读
			auto next_pro = nextToken();
			if (!next_pro.has_value())
				return {};
			if (next_pro.value().GetType() == TokenType::SEMICOLON) {
				addUninitializedVariable(next.value());
				continue;
				
			}
			//else addVariable(next.value());
			// '=':前面读过
			if (!next_pro.has_value() || next_pro.value().GetType() != TokenType::EQUAL_SIGN)
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrConstantNeedValue);
			// '<表达式>'
			...
			// ';'
			auto nexts = nextToken();
			if (!nexts.has_value() || nexts.value().GetType() != TokenType::SEMICOLON)
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
			// 生成一次 LIT 指令加载变量
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
		...
		if (next.value().GetType() != TokenType::PLUS_SIGN && next.value().GetType() != TokenType::MINUS_SIGN) {
			...
			else if(isInt32(next.value().GetValueString()))
				out = atoi(next.value().GetValueString().c_str());
			else return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrIntegerOverflow);
		}
		else if (next.value().GetType() == TokenType::MINUS_SIGN) {
			auto next1 = nextToken();
			if (next1.value().GetType() != TokenType::UNSIGNED_INTEGER)
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrIncompleteExpression);
			else if (isInt32(next1.value().GetValueString()))
				out = atoi(next1.value().GetValueString().c_str()) * (-1);
			else return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrIntegerOverflow);
		}
		else {
			auto next1 = nextToken();
			if (next1.value().GetType() != TokenType::UNSIGNED_INTEGER)
				...
				out = atoi(next1.value().GetValueString().c_str());
			else return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrIntegerOverflow);
		}
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
		...
		if (!next1.has_value() || next1.value().GetType() != TokenType::IDENTIFIER)
			return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNeedIdentifier);
		if (!isDeclared(next1.value().GetValueString()))
			return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNotDeclared);
		if (isConstant(next1.value().GetValueString()))
			return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrAssignToConstant);
		int index = _uninitialized_vars[next1.value().GetValueString()];
		_uninitialized_vars.erase(next1.value().GetValueString());
		_vars.insert(std::make_pair(next1.value().GetValueString(), index));
		//“=”
		auto next = nextToken();
		if (!next.has_value() || next.value().GetType() != TokenType::EQUAL_SIGN)
	...
	bool Analyser::isInt32(const std::string& s) {
		...
		*/
		if (s.size()> 10)
			...
			return true;
		else if (s.compare("2147483647") > 0)
			return false;
		else return true;
	}
}
