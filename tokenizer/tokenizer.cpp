	...
	std::pair<std::optional<Token>, std::optional<CompilationError>> Tokenizer::nextToken() {
		...
		while (true) {
			...
			case UNSIGNED_INTEGER_STATE: {
				...
				auto ch1 = current_char.value();



				// 如果读到的字符是数字，则存储读到的字符
				if (miniplc0::isdigit(ch1))
					ss << ch1;
				// 如果读到的是字母，则存储读到的字符，并切换状态到标识符
				else if (miniplc0::isalpha(ch1)) {
					ss << ch1;
			...
			case IDENTIFIER_STATE: {
				...
				auto cha = current_char.value();

				if (miniplc0::isalpha(cha) || miniplc0::isdigit(cha))
					ss << cha;
				else {
					unreadLast();
					if (ss.str() == "begin")
